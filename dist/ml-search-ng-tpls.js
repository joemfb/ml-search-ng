...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('/templates/ml-results.html',
    '<div ng-repeat="result in results"><h4><a ng-href="{{ result.link }}">{{ result.uri }}</a></h4><div class="matches"><div class="match" ng-repeat="match in result.matches"><em ng-repeat="text in match[\'match-text\'] track by $index"><span ng-class="{ highlight: !!text.highlight }">{{ text.highlight || text }}</span></em></div></div><hr></div>');
}]);
})();
